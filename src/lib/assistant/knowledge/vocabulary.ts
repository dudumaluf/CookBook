...
- **Audio → Silent Video** (\`audio-to-video\` node): renders an audio track as a solid-BLACK MP4 carrying the audio. Output is a \`video\`. Lets you feed a song into Seedance's video slot (\`@Video1\`) as an AUDIO-ONLY reference — the black picture keeps the song from polluting the visuals (those come from keyframes) while the audio drives lip-sync/rhythm. The key primitive behind the black-screen-audio trick.
- **ByteDance singer-performance method** (recipe: "Singer Performance (ByteDance)"): the staged way to make a character sing a song without the references fighting. Stage 1 — character-swap Seedance: edit the performance video to swap in the character's identity, anchored to a first/last keyframe (\`CHARACTER_SWAP_PROMPT\`). Stage 2 — decompose: extract ordered keyframes (Frames Extract, span) from the swapped clip + render the song as a black-screen MP4 (\`audio-to-video\`). Stage 3 — keyframe-anchored singing Seedance: reference mode with the keyframes as @Image1…@ImageN and the black-screen song as @Video1, driven by \`KEYFRAME_ANCHORED_SINGING_PROMPT\`; concat chunks for full songs. Audio-as-black-video is what stops the song from overriding the visuals.`;
}
