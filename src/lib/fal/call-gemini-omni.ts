...
export async function callGeminiOmni(
  args: CallGeminiOmniArgs,
): Promise<GeminiOmniSuccessResponse> {
  const { signal, ...body } = args;

  let submitRes: Response;
  try {
    submitRes = await authedFetch("/api/fal/gemini-omni", {
      method: "POST",
    ...
    throw new FalCallError(
      "Could not reach the Gemini Omni endpoint. Is the dev server running?",
      "network",
    );
  }
  ...
  const { requestId, endpoint } =
    (await submitRes.json()) as GeminiOmniSubmitResponse;
  ...
  while (Date.now() < deadline) {
    ...
    try {
      statusRes = await authedFetch("/api/fal/gemini-omni/status", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      if (consecutiveErrors >= MAX_CONSECUTIVE_POLL_ERRORS) {
        throw new FalCallError(
          "Lost connection while waiting for Gemini Omni. It may still finish on Fal — check the Gallery shortly.",
          "network",
        );
      }
      continue;
    ...
    const status = (await statusRes.json()) as GeminiOmniStatusResponse;
  ...
  throw new FalCallError(
    "Gemini Omni timed out. The job may still finish on Fal — try Run again in a moment.",
    "timeout",
  );
}
...
async function parseFalError(res: Response): Promise<FalCallError> {
  ...
  const message =
    payload?.error ?? `Gemini Omni call failed with HTTP ${res.status}`;
