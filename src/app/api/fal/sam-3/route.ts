...
import { segmentSam3Image } from "@/lib/fal/sam3-api";
import { sam3RequestSchema, type FalErrorResponse } from "@/lib/fal/types";

/**
 * POST /api/fal/sam-3 — SAM 3 image segmentation (Fal). Keeps FAL_KEY
 * server-only.
 */
...
export async function POST(req: NextRequest) {
  ...
  const parsed = sam3RequestSchema.safeParse(json);
  if (!parsed.success) {
  ...
  try {
    const result = await segmentSam3Image(parsed.data, req.signal, {
...
function mapErrorToResponse(err: unknown): NextResponse<FalErrorResponse> {
  ...
  console.error("[api/fal/sam-3] unexpected failure:", e);
  return errorResponse(500, "unknown", "SAM 3 segmentation failed");
}
