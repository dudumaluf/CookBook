...
import { submitHunyuan3d } from "@/lib/fal/hunyuan-3d-api";
import {
  hunyuan3dRequestSchema,
  type FalErrorResponse,
} from "@/lib/fal/types";

/**
 * POST /api/fal/hunyuan-3d — submit a Hunyuan 3D Pro image-to-3D job to Fal's queue.
 */
...
export async function POST(req: NextRequest) {
  ...
  const parsed = hunyuan3dRequestSchema.safeParse(json);
  if (!parsed.success) {
  ...
  try {
    const result = await submitHunyuan3d(parsed.data, req.signal, { userId: __auth.userId, accessToken: __auth.accessToken });
...
function mapErrorToResponse(err: unknown): NextResponse<FalErrorResponse> {
  ...
  console.error("[api/fal/hunyuan-3d] unexpected failure:", e);
  return errorResponse(500, "unknown", "Hunyuan 3D submit failed");
}
