...
import { getVeedSubtitlesResult } from "@/lib/fal/veed-subtitles-api";
import {
  veedSubtitlesStatusRequestSchema,
  type FalErrorResponse,
} from "@/lib/fal/types";

/** POST /api/fal/veed-subtitles/status — poll a queued VEED Subtitles job. */
...
export async function POST(req: NextRequest) {
  ...
  const parsed = veedSubtitlesStatusRequestSchema.safeParse(json);
  if (!parsed.success) {
  ...
  try {
    const result = await getVeedSubtitlesResult(
...
function mapErrorToResponse(err: unknown): NextResponse<FalErrorResponse> {
  ...
  console.error("[api/fal/veed-subtitles/status] unexpected failure:", e);
  return errorResponse(500, "unknown", "VEED Subtitles status check failed");
}
