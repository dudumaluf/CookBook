...
import { submitVeedSubtitles } from "@/lib/fal/veed-subtitles-api";
import {
  veedSubtitlesRequestSchema,
  type FalErrorResponse,
} from "@/lib/fal/types";

/** POST /api/fal/veed-subtitles — submit a VEED Subtitles job. */
...
export async function POST(req: NextRequest) {
  ...
  const parsed = veedSubtitlesRequestSchema.safeParse(json);
  if (!parsed.success) {
  ...
  try {
    const result = await submitVeedSubtitles(parsed.data, req.signal, { userId: __auth.userId, accessToken: __auth.accessToken });
...
function mapErrorToResponse(err: unknown): NextResponse<FalErrorResponse> {
  ...
  console.error("[api/fal/veed-subtitles] unexpected failure:", e);
  return errorResponse(500, "unknown", "VEED Subtitles submit failed");
}
