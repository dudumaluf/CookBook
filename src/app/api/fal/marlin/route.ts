...
import { submitMarlin } from "@/lib/fal/marlin-api";
import { marlinRequestSchema, type FalErrorResponse } from "@/lib/fal/types";

/** POST /api/fal/marlin — submit a Marlin video VLM job to Fal's queue. */
...
export async function POST(req: NextRequest) {
  ...
  const parsed = marlinRequestSchema.safeParse(json);
  if (!parsed.success) {
  ...
  try {
    const result = await submitMarlin(parsed.data, req.signal, { userId: __auth.userId, accessToken: __auth.accessToken });
...
function mapErrorToResponse(err: unknown): NextResponse<FalErrorResponse> {
  ...
  console.error("[api/fal/marlin] unexpected failure:", e);
  return errorResponse(500, "unknown", "Marlin submit failed");
}
