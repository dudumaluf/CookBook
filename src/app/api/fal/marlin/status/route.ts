...
import { getMarlinResult } from "@/lib/fal/marlin-api";
import {
  marlinStatusRequestSchema,
  type FalErrorResponse,
} from "@/lib/fal/types";

/** POST /api/fal/marlin/status — poll a queued Marlin job. */
...
export async function POST(req: NextRequest) {
  ...
  const parsed = marlinStatusRequestSchema.safeParse(json);
  if (!parsed.success) {
  ...
  try {
    const result = await getMarlinResult(
...
function mapErrorToResponse(err: unknown): NextResponse<FalErrorResponse> {
  ...
  console.error("[api/fal/marlin/status] unexpected failure:", e);
  return errorResponse(500, "unknown", "Marlin status check failed");
}
