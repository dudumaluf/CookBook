...
import { submitGeminiOmni } from "@/lib/fal/gemini-omni-api";
import {
...
/**
 * POST /api/fal/gemini-omni — SUBMIT a Gemini Omni job to Fal's queue
 * (Flash / 1.1 reference-to-video or Flash edit, ADR-0057). Keeps FAL_KEY
 * server-only; returns
 * `{ requestId, endpoint }` fast (no render wait). The client polls
 * `/api/fal/gemini-omni/status` until done.
 *
 *   200 -> GeminiOmniSubmitResponse ({ requestId, endpoint })
...
export async function POST(req: NextRequest) {
  ...
  const parsed = geminiOmniRequestSchema.safeParse(json);
  if (!parsed.success) {
  ...
  try {
    const result = await submitGeminiOmni(parsed.data, req.signal, {
...
function mapErrorToResponse(err: unknown): NextResponse<FalErrorResponse> {
  ...
  console.error("[api/fal/gemini-omni] unexpected failure:", e);
