...
import { getGeminiOmniResult } from "@/lib/fal/gemini-omni-api";
import {
  type FalErrorResponse,
  geminiOmniStatusRequestSchema,
} from "@/lib/fal/types";

/**
 * POST /api/fal/gemini-omni/status — poll a queued Gemini Omni Flash job
 * (ADR-0057). Body: { endpoint, requestId }. Returns `{ status: "pending" }`
 * while it renders, or `{ status: "done", videoUrl, ... }` once complete.
 *
 *   200 -> GeminiOmniStatusResponse
...
export async function POST(req: NextRequest) {
  ...
  const parsed = geminiOmniStatusRequestSchema.safeParse(json);
  if (!parsed.success) {
    return errorResponse(400, "invalid_request", "Invalid status payload");
  }

  try {
    const result = await getGeminiOmniResult(
...
function mapErrorToResponse(err: unknown): NextResponse<FalErrorResponse> {
  ...
  console.error("[api/fal/gemini-omni/status] unexpected failure:", e);
