...
import { getSam31VideoResult } from "@/lib/fal/sam31-video-api";
import {
  sam31VideoStatusRequestSchema,
  type FalErrorResponse,
} from "@/lib/fal/types";

/** POST /api/fal/sam-3-1-video/status — poll a queued SAM 3.1 Video job. */
...
export async function POST(req: NextRequest) {
  ...
  const parsed = sam31VideoStatusRequestSchema.safeParse(json);
  if (!parsed.success) {
  ...
  try {
    const result = await getSam31VideoResult(
...
function mapErrorToResponse(err: unknown): NextResponse<FalErrorResponse> {
  ...
  console.error("[api/fal/sam-3-1-video/status] unexpected failure:", e);
  return errorResponse(500, "unknown", "SAM 3.1 Video status check failed");
}
