...
import { submitSam31Video } from "@/lib/fal/sam31-video-api";
import {
  sam31VideoRequestSchema,
...
export async function POST(req: NextRequest) {
  ...
  const parsed = sam31VideoRequestSchema.safeParse(json);
  if (!parsed.success) {
  ...
  try {
    const result = await submitSam31Video(parsed.data, req.signal, {
...
function mapErrorToResponse(err: unknown): NextResponse<FalErrorResponse> {
  ...
  console.error("[api/fal/sam-3-1-video] unexpected failure:", e);
  return errorResponse(500, "unknown", "SAM 3.1 Video submit failed");
}
