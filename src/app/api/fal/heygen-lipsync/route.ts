...
import { submitHeygenLipsync } from "@/lib/fal/heygen-lipsync-api";
import {
  heygenLipsyncRequestSchema,
  type FalErrorResponse,
} from "@/lib/fal/types";

/** POST /api/fal/heygen-lipsync — submit a HeyGen Lipsync Precision job. */
...
export async function POST(req: NextRequest) {
  ...
  const parsed = heygenLipsyncRequestSchema.safeParse(json);
  if (!parsed.success) {
  ...
  try {
    const result = await submitHeygenLipsync(parsed.data, req.signal, { userId: __auth.userId, accessToken: __auth.accessToken });
...
function mapErrorToResponse(err: unknown): NextResponse<FalErrorResponse> {
  ...
  console.error("[api/fal/heygen-lipsync] unexpected failure:", e);
  return errorResponse(500, "unknown", "HeyGen Lipsync submit failed");
}
