...
import {
  heygenLipsyncStatusRequestSchema,
  type FalErrorResponse,
} from "@/lib/fal/types";

/** POST /api/fal/heygen-lipsync/status — poll a queued HeyGen Lipsync job. */
...
export async function POST(req: NextRequest) {
  ...
  const parsed = heygenLipsyncStatusRequestSchema.safeParse(json);
  if (!parsed.success) {
...
function mapErrorToResponse(err: unknown): NextResponse<FalErrorResponse> {
  ...
  console.error("[api/fal/heygen-lipsync/status] unexpected failure:", e);
  return errorResponse(500, "unknown", "HeyGen Lipsync status check failed");
}
