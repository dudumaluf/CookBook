...
import { restyleTelestyleV2 } from "@/lib/fal/telestyle-v2-api";
import {
  telestyleV2RequestSchema,
  type FalErrorResponse,
} from "@/lib/fal/types";

/**
 * POST /api/fal/telestyle-v2 — TeleStyle V2 style transfer (Fal). Keeps
 * FAL_KEY server-only.
 */
...
export async function POST(req: NextRequest) {
  ...
  const parsed = telestyleV2RequestSchema.safeParse(json);
  if (!parsed.success) {
  ...
  try {
    const result = await restyleTelestyleV2(parsed.data, req.signal, {
...
function mapErrorToResponse(err: unknown): NextResponse<FalErrorResponse> {
  ...
  console.error("[api/fal/telestyle-v2] unexpected failure:", e);
  return errorResponse(500, "unknown", "TeleStyle V2 style transfer failed");
}
