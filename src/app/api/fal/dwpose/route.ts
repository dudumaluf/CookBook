...
import { submitDwpose } from "@/lib/fal/dwpose-api";
import { dwposeRequestSchema, type FalErrorResponse } from "@/lib/fal/types";

/** POST /api/fal/dwpose — submit a DWPose pose-estimation job. */
...
export async function POST(req: NextRequest) {
  ...
  const parsed = dwposeRequestSchema.safeParse(json);
  if (!parsed.success) {
  ...
  try {
    const result = await submitDwpose(parsed.data, req.signal, {
...
function mapErrorToResponse(err: unknown): NextResponse<FalErrorResponse> {
  ...
  console.error("[api/fal/dwpose] unexpected failure:", e);
  return errorResponse(500, "unknown", "DWPose submit failed");
}
