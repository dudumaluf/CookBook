...
import { submitH3Max } from "@/lib/fal/h3-max-api";
import { type FalErrorResponse, h3MaxRequestSchema } from "@/lib/fal/types";

/**
 * POST /api/fal/h3-max — SUBMIT a MiniMax H3 Max image-to-video job to
 * Fal's queue (ADR-0057). Keeps FAL_KEY server-only; returns
 * `{ requestId, endpoint }` fast (no render wait). The client polls
 * `/api/fal/h3-max/status` until done.
 *
 *   200 -> H3MaxSubmitResponse ({ requestId, endpoint })
...
export async function POST(req: NextRequest) {
  ...
  const parsed = h3MaxRequestSchema.safeParse(json);
  if (!parsed.success) {
  ...
  try {
    const result = await submitH3Max(parsed.data, req.signal, {
...
function mapErrorToResponse(err: unknown): NextResponse<FalErrorResponse> {
  ...
  console.error("[api/fal/h3-max] unexpected failure:", e);
