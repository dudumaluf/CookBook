...
import { getScribeV2Result } from "@/lib/fal/scribe-v2-api";
import {
  scribeV2StatusRequestSchema,
  type FalErrorResponse,
} from "@/lib/fal/types";

/**
 * POST /api/fal/scribe-v2/status — poll a queued Scribe V2 job.
 */
...
export async function POST(req: NextRequest) {
  ...
  const parsed = scribeV2StatusRequestSchema.safeParse(json);
  if (!parsed.success) {
  ...
  try {
    const result = await getScribeV2Result(
...
function mapErrorToResponse(err: unknown): NextResponse<FalErrorResponse> {
  ...
  console.error("[api/fal/scribe-v2/status] unexpected failure:", e);
  return errorResponse(500, "unknown", "Scribe V2 status check failed");
}
