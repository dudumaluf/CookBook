...
import {
  scribeV2RequestSchema,
  type FalErrorResponse,
} from "@/lib/fal/types";

/**
 * POST /api/fal/scribe-v2 — submit ElevenLabs Scribe V2 STT to Fal's queue.
 */
...
export async function POST(req: NextRequest) {
  ...
  const parsed = scribeV2RequestSchema.safeParse(json);
  if (!parsed.success) {
...
function mapErrorToResponse(err: unknown): NextResponse<FalErrorResponse> {
  ...
  console.error("[api/fal/scribe-v2] unexpected failure:", e);
  return errorResponse(500, "unknown", "Scribe V2 failed");
}
