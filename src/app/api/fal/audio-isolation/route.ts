...
import { submitAudioIsolation } from "@/lib/fal/audio-isolation-api";
import {
  audioIsolationRequestSchema,
  type FalErrorResponse,
} from "@/lib/fal/types";

/**
 * POST /api/fal/audio-isolation — submit ElevenLabs audio isolation to Fal's queue.
 */
...
export async function POST(req: NextRequest) {
  ...
  const parsed = audioIsolationRequestSchema.safeParse(json);
  if (!parsed.success) {
  ...
  try {
    const result = await submitAudioIsolation(parsed.data, req.signal, { userId: __auth.userId, accessToken: __auth.accessToken });
...
function mapErrorToResponse(err: unknown): NextResponse<FalErrorResponse> {
  ...
  console.error("[api/fal/audio-isolation] unexpected failure:", e);
  return errorResponse(500, "unknown", "Audio isolation failed");
}
