...
- **Last shipped slice**: **ADR-0033 + Slice 5.6.2 — production-first development + dynamic aspect ratio in node previews**. ADR-0033 cristalliza a regra "production-first" (Vercel + Supabase + Higgsfield + Fal canonical stack; webhooks são primários; smoke em prod é parte da definição-de-pronto). Slice 5.6.2 corrige o problema concreto: previews de imagem agora refletem o aspect ratio real (9:16 não vira mais quadrado esmagado). Captura `width / height` no upload via `extractImageDimensions(file)`, com fallback `<img onLoad>` pra assets antigos. Higgsfield placeholder/single-result usa `config.aspectRatio` direto; grid 2×2 mantém quadrado. Tests 605 / 605.
