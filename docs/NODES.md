...
Previz stage for reference-to-video. The grid is the floor; add a Plane if you want a visible ground mesh. Add primitives (capsule = person) or import GLB/OBJ/FBX, keyframe **pose keys** (one tick per object per time; inspector circles key P/R/S), or prompt the in-node scene agent (it edits existing ids; camera, lookAt, and FOV are independent). Multi-cam shot list drives Shot view and playblast cuts. Presets (wide/medium/close/OTS/profile) key the camera. Dope sheet is one row per object. Figure + walk macros; sample VAT character (in-place clips). Drag any object (or Camera / Look at) onto another to parent them so they follow it. Optional per-object color. **Instancer** clones its children (linear / grid / scatter); parent **Effectors** onto it to jitter P / R / S. **Shot** looks through the playblast camera; **Orbit** lets you click the camera or the look-at handle and move them (optionally linked). Timeline keys drag to retime (playhead stays); click a key to edit its stored value. Click the bar to seek. Escape deselects; W/E/R gizmos; Key / Auto-key. In the fullscreen editor, Delete/Backspace remove the selected object(s) or key (hover trash in the outliner; Shift / ⌘-click multi-selects); Cmd/Ctrl+Z undoes scene edits including a bad prompt. Number fields type or drag-scrub. Scrub / play on the node body. Run playblasts an MP4. Wire Hunyuan meshes into `mesh`. No lights or rigs.
