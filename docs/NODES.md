...
Previz stage for reference-to-video. The grid is the floor; add a Plane if you want a visible ground mesh. Add primitives (capsule = person) or import GLB/OBJ/FBX, keyframe PSR + camera (position / lookAt / fov), or prompt the in-node scene agent (it edits existing ids; camera, lookAt, and FOV are independent). Drag Camera / Look at onto an object to parent them so they follow it. **Shot** looks through the playblast camera; **Orbit** lets you click the camera or the look-at handle and move them (optionally linked). Timeline keys drag to retime (playhead stays); click a key to edit its stored value. Click the bar to seek. In the fullscreen editor, Delete/Backspace remove the selected object(s) or key (hover trash in the outliner; Shift / ⌘-click multi-selects); Cmd/Ctrl+Z undoes scene edits including a bad prompt. Number fields type or drag-scrub. Scrub / play on the node body. Run playblasts an MP4. Wire Hunyuan meshes into `mesh`. No lights or rigs.
