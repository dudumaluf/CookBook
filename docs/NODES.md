...
Previz stage for reference-to-video. Add primitives (capsule = person) or import GLB/OBJ/FBX, keyframe PSR + camera (position / lookAt / fov), or prompt the in-node scene agent (it edits existing ids; camera and lookAt are independent). Drag Camera / Look at onto an object to parent them so they follow it. **Shot** looks through the playblast camera; **Orbit** lets you click the camera or the look-at handle and move them (optionally linked). Timeline keys drag to retime; click a key to edit its stored value. In the fullscreen editor, Delete/Backspace remove the selected object or key (hover trash in the outliner); Cmd/Ctrl+Z undoes scene edits including a bad prompt. Scrub / play on the node body. Run playblasts an MP4. Wire Hunyuan meshes into `mesh`. No lights or rigs.
