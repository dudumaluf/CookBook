...
**Fix** ([`node-fal-sam31-video.tsx`](src/components/nodes/node-fal-sam31-video.tsx)). Drive the drag from **`window`** `pointermove`/`pointerup` listeners (a `useEffect` reading a `dragStart` ref + a `draftRef` mirror), **and** `setPointerCapture(e.pointerId)` on the frame at pointer-down so the OS pins the pointer to the frame even if an overlay (e.g. a browser extension) sits on top. The window listeners are the real driver — they always see the bubbled moves — and the capture is best-effort (a throw is caught and harmless). The frame keeps only `onPointerDown`. Mirrors the composer stage's window-level gesture loop.

**Verification — real trusted input, not synthetic.** First attempt only proved JS-`dispatchEvent` (untrusted) worked, which is not the same as OS input. Re-verified properly: read Base UI's `DialogPopup` source (it has **no pointer handlers** and does **not** capture — my first "capture-steal" theory was wrong), then drove a **real mouse drag with Playwright** (`mouse.down/move/up`, channel `chrome`) against the editor inside the actual Dialog, in **both `next dev` and a production `next build`**. Both: draft draws live (45%×45%) and commits `box={0.25,0.25,0.70,0.70}`; real events show `pointerdown ×1`, `pointermove ×9` reaching `window`, `pointerup ×1`. So the code is correct in prod — a lingering user-side failure is a **stale cached bundle** (reload across a deploy) or a **page-overlay browser extension**, not the app.

**Tests (±).** [`sam31-mask-editor.test.tsx`](tests/component/nodes/sam31-mask-editor.test.tsx): drag test dispatches move/up on `window` (never the frame) to pin the window-driven path; a test asserts the dashed draft renders live mid-drag (40%×40%); click-sized non-drag ignored. 3 pass; full suite + `tsc` + lint green.

**Hardening (follow-up, same day).** The user still reported no box despite the verified-in-prod fix, so the gesture is now bulletproofed against the two plausible user-side causes that synthetic + Playwright runs can't reproduce: (1) the `window` listeners moved to the **capture phase** (`addEventListener(…, true)`) so they fire *before* any bubble-phase `stopPropagation()` a browser extension might run; (2) the **mouse event family** (`mousedown`/`mousemove`/`mouseup`) is now bound alongside the pointer family via a shared `beginDrag` + idempotent move/up handlers, so the drag still works even if an extension suppresses pointer events outright (the two families firing together is a harmless no-op re-seed). Plus an **opt-in on-screen diagnostic**: append `?samdebug` to the URL and the editor shows a live `dbg2 · down/move/drag/ev` readout (mirrored into state, gated so it's zero-cost otherwise) — if a user's drag fails, that readout pinpoints exactly which stage breaks (no pointerdown → overlay; down but no move → events blocked; moves but no box → render). +1 test: drag committed via the **mouse-only fallback** (no pointer events at all). 4 pass; `tsc` + lint green.
...
**Timeline panel** ([`composer-timeline.tsx`](src/components/nodes/composer/composer-timeline.tsx), new). A transport (play/pause + a `0.0s / 5.0s` readout + a `−/+` length stepper + fps), a tick **ruler** you press/drag to scrub, and **one track per layer** (topmost on top). Each clip is positioned by its `layerSpan`; drag the **body** to re-sequence (move), the **left/right edges** to trim (head trim advances a video's `trimIn` so source frames stay pinned; image head trim just moves start), and the **top-corner dots** to set **fade in/out** (ramps render live on the clip). A **playhead** line tracks the clock across every track. All drags route through pure, unit-tested mutators `moveClip` / `trimClipStart` / `trimClipEnd` / `setFade` (+ `ensureTiming` / `clampFadesTo`) off a *snapshot captured at gesture start*, so live patches never compound.

**Transport + live preview.** The editor owns the master clock ([`composer-editor.tsx`](src/components/nodes/composer/composer-editor.tsx)): an rAF loop advances `playheadMs` and **loops** at `durationMs`; **space** toggles play; scrubbing pauses. The stage ([`composer-stage.tsx`](src/components/nodes/composer/composer-stage.tsx)) now honours the playhead in timeline mode — a layer only paints inside its span and at its **fade-multiplied** opacity (`layerActiveAt` / `layerOpacityAt`), and `<video>` layers **play in sync** (registered via a ref map; paused → seek+hold the source frame, playing → run and re-seek only on >0.34s drift). It's a best-effort *preview* — Run stays frame-exact (ADR-0091). Precise numeric control lands in the **properties panel** ([`composer-properties-panel.tsx`](src/components/nodes/composer/composer-properties-panel.tsx)): a doc **Timeline** section (duration s + fps) and a per-layer **Timing** block (Start / End / Fade in / Fade out, in seconds), both shown only in timeline mode and routed through the same mutators.

**Tests (+11).** [`composer.test.ts`](tests/unit/types/composer.test.ts): `ensureTiming` full-span, `moveClip` clamp/preserve-duration, `trimClipStart` video-advances-trimIn vs. image-no-trim, `trimClipEnd` clamp, `setFade` sum-≤-duration + zero-drops (+6). [`composer-timeline.test.tsx`](tests/component/nodes/composer-timeline.test.tsx, new): transport readout + a clip per layer, play toggle, ruler scrub (px→ms), clip-body drag re-sequences (duration preserved), length stepper (+5). Full suite green (2628); `tsc` + lint clean.

## 2026-06-28 — Composer timeline: a wired video renders as real motion (not a still); `out` flips image→video

The user was clear: *"I want a video not to be a still from the video … each layer is a layer **and** a layer in a timeline — sequence, trim, control duration, fade in/out, move/scale/rotate … like a basic After Effects."* Slice 1 (ADR-0089) only sampled a poster frame; this ships the **engine** for a real timeline render (ADR-0091, supersedes that). Wiring a video into the Composer now produces a true per-frame composited MP4.

**What ships (model + export).** `ComposerDocument` gains `durationMs` (0/absent = IMAGE mode → PNG, `> 0` = TIMELINE mode → MP4) + `fps` (default 30). `LayerTiming` activates — `{ startMs, endMs, trimInMs?, fadeInMs?, fadeOutMs? }` — with pure, unit-tested helpers `layerSpan` / `layerActiveAt` / `layerOpacityAt` (base × fade) / `layerSourceTimeMs` / `docFrameCount` / `isTimelineMode` ([`composer.ts`](src/types/composer.ts)). The still + video renderers now share ONE `paintOneLayer` over a `Drawable` abstraction, so transforms/blend/alpha-luma masks are pixel-identical in both; `renderCompositeVideo` decodes stills once, opens a forward-seeking `VideoFrameReader` (mediabunny `getSample`, holds last frame past a clip's end) per video layer/mask, composites every output frame, and encodes via `CanvasSource`→`Mp4OutputFormat` — a video frame draws through `VideoSample.toCanvasImageSource()` so masks/rotation reuse the exact code ([`compose-composer.ts`](src/lib/media/compose-composer.ts)). The node's `getOutputs` flips `out` image↔video on `durationMs`; **wiring a video auto-enters timeline mode** sized to the longest clip (5s fallback). Reactive preview stays cheap — a single **poster** frame for the node body ("poster · Run to render video"); the full encode is Run-only, and the body shows the baked `<video>` after a Run ([`node-composer.tsx`](src/components/nodes/node-composer.tsx)). Persistence: additive, funnels through `sanitizeComposerDocument`; store **v15 → v16**.

**Timeline UI (shipped — see the entry above).** The transport + per-layer clip track (drag-to-move, edge-trim, fade handles) + fps/duration controls landed in the next commit, so the timing fields this renderer honours are now directly editable. (Keyframed transforms remain out of scope; transforms stay static per layer. An export-progress meter is still a nice-to-have.)

**Tests (+).** [`composer.test.ts`](tests/unit/types/composer.test.ts): fps/duration clamps, `docFrameCount`, `layerSpan` clamping, half-open `layerActiveAt`, fade-in/out + base-multiplied `layerOpacityAt`, `layerSourceTimeMs` trim mapping, timeline-field sanitisation (+15 cases). [`node-composer.test.ts`](tests/unit/nodes/node-composer.test.ts): `getOutputs` image↔video; timeline Run encodes a video + uploads mp4; timeline preview renders a poster (no encode/upload). [`node-composer.test.tsx`](tests/component/nodes/node-composer.test.tsx): wiring a video auto-sets `durationMs`. Full suite green (2617).

## 2026-06-28 — Fix: SAM 3.1 Video 500 finally root-caused — Fal's model crashes on point prompts; node is now box + text only

The persistent `Fal (500): Internal Server Error` when marking the object (a box **+** include/exclude points) — *not* fixed by ADR-0088's endpoint move or the earlier `object_id`/coded-vs-display fixes. Instead of guessing a fifth time, I probed the **live** endpoint (FAL_KEY from `.env.local`, ~15 calls, reading the *queued-job result body* where the model's real error lives — submit always 200s). Verdict: **box prompts ✅, text prompts ✅, box+text ✅; point prompts ❌ crash on BOTH `/video` and `/video-rle`** (any id field — `object_id`/`obj_id`/none — even in-bounds `(10,10)`), and **box+points is explicitly rejected** (`"SAM3.1 does not support point prompts and box prompts on the same frame"`). The user's box+points marking hit exactly that. Point prompts are simply unusable on Fal's SAM 3.1 video today. ADR-0090 (supersedes ADR-0088's "points work / `frame_index` harmless" — `frame_index` was a phantom field; `/video` consumes `PointPromptBase`/`BoxPromptBase`, which don't model it).

**What ships.** The mask editor is **box-only** — Include/Exclude point tools, the tool switcher, and the `points` config field are gone; the big modal preview stays, drag draws a box, drag again replaces it ([`node-fal-sam31-video.tsx`](src/components/nodes/node-fal-sam31-video.tsx); `sam31VisualPromptsToPixels` → `sam31BoxToPixels`). `buildInput` **never forwards `point_prompts`** and drops `frame_index`, so the crash can't be re-triggered even by a stray caller — box keeps `object_id` (default 1) ([`sam31-video-api.ts`](src/lib/fal/sam31-video-api.ts)). A text prompt may still accompany the box (verified ✅). `pointPrompts` stays inert in the request schema (backward-compat; a future `/video-rle`+RLE-decode path could revive it if Fal fixes points). Node description + settings copy + editor tips reworded to "draw a box / describe".

**Tests (±).** [`sam31-video-build-input.test.ts`](tests/unit/fal/sam31-video-build-input.test.ts): box gets `object_id` + no `frame_index`; point prompts are never forwarded. [`node-fal-sam31-video.test.ts`](tests/unit/nodes/node-fal-sam31-video.test.ts): `sam31BoxToPixels` (box-only), box maps against display dims with no `frameIndex`, visual mode forwards no points, throws "Draw a box…". [`sam31-mask-editor.test.tsx`](tests/component/nodes/sam31-mask-editor.test.tsx): editor drops the `points` prop, trigger is "Draw a box around the object". [`sam31-video-route.test.ts`](tests/unit/fal/sam31-video-route.test.ts): box-centric accept/forward + negative-box 400. SAM suites (29) + `tsc` + lint green.

**Verification:** the box path is proven end-to-end — two live jobs (text+box and box-only) **completed** and returned a rendered mask video. The exact failing setup (box + text "person", minus points) is now the ✅ text+box path.

## 2026-06-28 — Composer video layers, Slice 1: wire a video → it composites (as a sampled frame)

Wiring a **video** into a second Composer layer did nothing — no layer, no preview, no error. Root cause was three silent walls: the `layer-N` sockets were typed `image`, the input resolver (`firstImageRef`) dropped any non-image output so a video never became a layer, and the renderer decodes stills (`createImageBitmap` can't read an mp4). This is Slice 1 of a 5-slice arc to full **video layers + a timeline** (ADR-0089); it both unblocks "drop a video as a layer" today and lays the substrate (media-aware inputs + frame-at-time sampling) the timeline rides on.

**What ships.** Layer sockets are now `dataType: "any"` (the universal wildcard — images **and** videos wire cleanly, manually + via the assistant). A wired handle resolves to a `ComposerInputRef { url, mediaType, width?, height?, durationMs? }` via new `firstMediaRef` (narrows image **or** video); `resolveLayerMediaType` / `resolveLayerMediaTypes` tag each layer, preferring the live wired kind over a stale `source.mediaType`. `renderComposite` routes video layers through `extractFrame(url, {atMs})` → `createImageBitmap` at a shared `atSec` (poster/0 today, the playhead later) — so a video **composites as a still frame**; output stays a PNG. The editor stage + layers-panel thumbnail preview a video layer as a muted, paused `<video>` poster. `compositeCacheKey` now encodes media kind + `atSec`; image-only docs key identically to before (no cache churn). `getOutputs` will flip `out` image→video in Slice 2 — not yet.

**Files.** [`composer.ts`](src/types/composer.ts) (`ComposerInputRef`, `firstMediaRef`, `resolveLayerMediaType(s)`, widened resolver signatures), [`compose-composer.ts`](src/lib/media/compose-composer.ts) (`loadDrawableBitmap` video branch + `mediaTypes`/`atSec`), [`node-composer.tsx`](src/components/nodes/node-composer.tsx) (`any` sockets, media-aware `useComposerInputs` + `execute`, video-tagged auto-add), stage / layers-panel / properties-panel / editor (`inputs` → `ComposerInputRef`, `<video>` poster). Audio dropped in v1; fps default 30, duration default = longest video layer (Slice 2+).

**Tests (+).** [`composer.test.ts`](tests/unit/types/composer.test.ts): `firstMediaRef` image+video, live-kind-wins, `resolveLayerMediaTypes` map. [`node-composer.test.ts`](tests/unit/nodes/node-composer.test.ts): a video input threads `mediaTypes: video` into `renderComposite` + the cache key. [`node-composer.test.tsx`](tests/component/nodes/node-composer.test.tsx): a wired video output auto-adds a `mediaType: "video"` layer. Full suite green (2599+).

## 2026-06-27 — Fix: SAM 3.1 Video was calling `/video-rle` (returns RLE arrays, no video) → now `/video`

The whole reason the mask-tracked crop workflow never worked. A real run **succeeded at Fal** but the poll 502'd with **`SAM 3.1 Video returned no video URL (output keys: rle, boundingbox_frames_zip, metadata, scores, boxes)`**. Root cause: we targeted **`fal-ai/sam-3-1/video-rle`**, which returns **run-length-encoded mask arrays + per-frame boxes + scores** — *never* a rendered `video`. The SAM node re-hosts a *video* and Object Track Crop decodes a *video*, so the pipeline had nothing to consume. We were misled by Fal's OpenAPI: the `/video-rle` output schema is a **stale copy** of the `/video` one (both advertise a `video` File), so the spec looked fine — only a live run exposed the real shape. (The earlier `object_id` + coded-vs-display fixes were real but orthogonal: they made the *submit* succeed; the *output type* was always wrong.)

**Fix.** `SAM31_VIDEO_ENDPOINT` → **`fal-ai/sam-3-1/video`** (the rendered-video endpoint) ([`types.ts`](src/lib/fal/types.ts)). Verified against the live Fal OpenAPI that `/video`'s **input is identical** to what `buildInput` already sends (`video_url`, `prompt`, `point_prompts`, `box_prompts` with `object_id`, `apply_mask`, `detection_threshold`, `max_num_objects`) and its **output `{ video: File }`** is exactly what `extractVideoUrl` + the Object Track Crop → Track Recompose pipeline already consume. **One-constant change — no pipeline rework.** Stale "RLE / renders a mask video" comments corrected in [`sam31-video-api.ts`](src/lib/fal/sam31-video-api.ts) + the route; GLOSSARY + assistant vocabulary now name `/video` (and warn off `/video-rle`). ADR-0088 records it; ADR-0079's wrong "`/video-rle` renders a mask video" premise is corrected in place.

**Tests (±).** Endpoint strings flipped to `fal-ai/sam-3-1/video` in [`sam31-video-route.test.ts`](tests/unit/fal/sam31-video-route.test.ts) (submit + status mocks) and [`node-fal-sam31-video.test.ts`](tests/unit/nodes/node-fal-sam31-video.test.ts) (the `callSam31Video` mock). `buildInput` payload assertions unchanged (the wire shape didn't move). Still needs a live re-run to confirm the segmented video comes back (the Fal call can't be exercised in tests).
