...
**Still to come (same ADR, next slices).** The **timeline UI**: a transport (play/pause + scrub playhead) and a per-layer clip track with drag-to-move, edge-trim (`trimIn`/duration), and fade handles — so the timing fields the renderer already honours become directly editable. Plus fps/duration controls + an export-progress meter. (Keyframed transforms remain out of scope; transforms stay static per layer.)
