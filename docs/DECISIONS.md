...
- **Consequences**:
  ...
  - ~~**Image-to-video for Mini/Fast is deferred.**~~ Resolved same-day — see the update below.
...
## ADR-0091: Composer timeline (Phase 4) — a video is a real per-frame render, not a still; `out` flips image↔video with `doc.durationMs`

- **Status**: Accepted (2026-06-28); supersedes ADR-0089's "composite a sampled frame now, encode per-frame later".

- **Context**: ADR-0089 (Phase 3) let a video wire into a layer but composited a single *poster frame* — the flattened output stayed a PNG. The user was explicit: *"I want a video not to be a still from the video … each layer is a layer in the layer system but also a layer in a timeline … sequence, trim, control duration, fade in/out, move/scale/rotate … manipulate as one would in a basic After Effects."* So the Composer needs a real time dimension: per-layer clips on a timeline, a motion render, and (next) a timeline editing surface.

- **Decision**: Give the document a timeline and render it frame-by-frame to an MP4.
  - **Model (pure, `src/types/composer.ts`)**: `ComposerDocument` gains optional `durationMs` (0/absent = IMAGE mode, the original flatten; `> 0` = TIMELINE mode) + `fps` (default 30). `LayerTiming` activates: `{ startMs, endMs, trimInMs?, fadeInMs?, fadeOutMs? }` — `start..end` is the clip's lifetime on the OUTPUT timeline, `trimIn` is the in-point WITHIN the source (video), fades ramp opacity at the head/tail. Pure helpers — `layerSpan` (a layer with no timing spans the whole doc), `layerActiveAt`, `layerOpacityAt` (base × fade), `layerSourceTimeMs` (`trimIn + (t − start)`), `docDurationMs`/`docFps`/`docFrameCount`/`isTimelineMode` — are unit-tested and shared by the renderer + (coming) timeline UI.
  - **One compositor, two drivers (`compose-composer.ts`)**: the per-layer paint (transform + blend + alpha/luma mask) is refactored behind a `Drawable = { src: CanvasImageSource; w; h }` + a single `paintOneLayer`, so the still flatten and the per-frame video encode are **pixel-identical** — the only per-frame variables are the resolved drawable + fade-applied opacity. `renderCompositeVideo` decodes image/solid layers + image masks ONCE, opens one `VideoFrameReader` (mediabunny `Input` + `VideoSampleSink.getSample`, forward-seek, holds the last frame past a clip's end) per video layer / video mask, composites every output frame, and encodes via `CanvasSource`→`Mp4OutputFormat` (the proven track-crop path). A video frame draws through `VideoSample.toCanvasImageSource()` (a `CanvasImageSource`), so masks/rotation reuse the exact same code.
  - **Output type follows the doc (`getOutputs`)**: `out` is `video` when `durationMs > 0`, else `image`. Wiring a VIDEO auto-enters timeline mode sized to the longest fresh clip (falling back to 5s when a clip's duration is unknown) — so "whatever media we plug in" becomes a motion render with zero ceremony.
  - **Reactive preview stays cheap**: a full per-frame encode per keystroke is infeasible, so in timeline mode the reactive preview renders a single **poster frame** (one `renderComposite` at t=0) for the node body — labelled "poster · Run to render video" — and the full encode is **Run-only** (Run-cost is the WebCodecs encode, not money). The node body shows the baked `<video>` after a Run, the poster before it.
  - **Persistence**: all fields are additive + funnel through the existing `sanitizeComposerDocument` walk; store bumped v15 → **v16** to re-run the (idempotent) sanitiser. Image-mode docs round-trip unchanged (no `durationMs`/`fps` written).

- **Why this shape**: Sharing one `paintOneLayer` between still + video is the anti-drift guarantee — a blend/mask/transform fix lands in both paths at once. Driving by OUTPUT frames (not by one source's frames, like track-crop) is what lets *multiple* sources at *different* framerates mix onto one timeline. Keeping the heavy encode Run-only mirrors how the Fal video nodes behave (no reactive video), and a poster keeps the node body honest meanwhile.

- **Consequences / next**: This ADR now covers BOTH the *engine* (real motion out + the timing model) and the **timeline editing UI** that drives it. The UI (shipped same day): `composer-timeline.tsx` — a transport (play/pause + scrub + length stepper + fps), a ruler, and a per-layer clip track with **drag-to-move** (`moveClip`), **edge-trim** (`trimClipStart` advances a video's `trimIn`; `trimClipEnd` sets duration), and **fade handles** (`setFade`, clamped so in+out ≤ clip duration). All drags route through pure mutators (`ensureTiming`/`clampFadesTo`/`moveClip`/`trimClip*`/`setFade`) off a snapshot taken at gesture start, so live patches never compound. The editor owns a master rAF clock (`playheadMs`, loops at `durationMs`, space=play); the stage honours it (layers paint only inside their span at fade-multiplied opacity, `<video>` layers play in sync — best-effort preview, Run is frame-exact); the properties panel adds precise numeric Timeline (duration/fps) + per-layer Timing (start/end/fades) fields. **Still nice-to-have**: an export-progress meter. **Out of scope**: keyframed transforms (animated move/scale/rotate) — transforms stay static per layer.


## ADR-0092: Gemini Omni Flash node implements the documented `reference-to-video` schema (image + text in), not the "omni" marketing

- **Status**: Accepted (2026-07-04).

- **Context**: The user asked to add Google's **Gemini Omni Flash** (`google/gemini-omni-flash/reference-to-video`) as a node. Its Fal page is framed as "omni" — the prose says it "accepts text, images, audio, and video together as input" and tags it `lipsync`. But the **documented input schema** for the `reference-to-video` endpoint is just `prompt` (required) + `image_urls` (required) + `aspect_ratio` (`16:9`/`9:16`) + `duration` (3–10s). There is **no `audio_url` / `video_url` field**. The audio the model is known for comes OUT (native soundtrack on the generated clip), not in. Implementing the marketing rather than the schema would mean inventing request fields Fal's `.strict()` validator (and ours) would reject.

- **Decision**: Build to the **documented schema** — image + text in, video (with native audio) out — and record the discrepancy rather than silently guessing. Concretely:
  - **Endpoint + transport**: `google/gemini-omni-flash/reference-to-video` via the Fal **queue** (submit + poll, ADR-0057), the same wrapper shape as Seedance / DWPose — `gemini-omni-api.ts` (server), `call-gemini-omni.ts` (client, 20-min deadline, rides out 5 transient poll blips), `/api/fal/gemini-omni` + `/status` routes (`FAL_KEY` server-only). Video renders are minutes-long; a held blocking request would drop them.
  - **Reference images**: `image_urls` is required (≥1). The node exposes auto-growing numbered `<IMAGE_REF_0>`..sockets (the model's own inline role-binding tag scheme) plus one `<IMAGE_REF[]>` array socket (wire a Frames Extract / Array straight in) — mirroring the Seedance `@Image[]` pattern but with **no friendly-name rewrite** (kept out for simplicity; the user types the tag the socket label shows). Capped at **4** images (`GEMINI_OMNI_MAX_IMAGES`): the schema documents no hard max, but reference-to-video role binding uses a handful (the docs' own examples wire two). One-line bump if a workflow needs more and Fal accepts it.
  - **No seed → non-cache-busting**: the endpoint takes no seed, so the node omits `isCacheBusting`; identical inputs hit the run cache instead of re-spending — consistent with the user's "only generate something new" rule. A new variation means changing the prompt/refs.
  - **Persistence is free**: `category: "ai-video"` means `generation-sync` auto-rehosts the Fal video to Supabase + inserts a content-hash-deduped `cookbook_generations` row (ADR-0035). No node-level upload code.

- **Why this shape**: Schema-over-marketing is the anti-confabulation guarantee — the node can only send fields Fal actually accepts, so a "why did my audio input do nothing?" bug can't exist (there is no audio input to mis-wire). Reusing the queue + ref-socket + persistence conventions keeps the node a thin adapter (~1 wrapper, 1 caller, 2 routes, 1 component) rather than new infrastructure.
