...
- **Consequences / next**: This slice delivers the *engine* (real motion out + the timing model) — wiring a video now yields a true composited clip, not a freeze-frame. Still **to come** (same ADR, next slices): the **timeline UI** (transport/playhead + a per-layer clip track with drag-to-move, edge-trim, and fade handles) so `start/end/trimIn/fades` are directly editable, plus fps/duration controls and an export-progress meter. Until then, a wired video plays full-span from its start (shorter clips hold their last frame); the timing fields are honoured by the renderer the moment the UI writes them. Keyframed transforms (animated move/scale/rotate) are explicitly out of scope for now — transforms stay static per layer.
