...
- **Slice 5.5 — Image Iterator + Text Iterator internal multi-storage** — migrate `Image Iterator` from "multi-edge into one input port" to "drop N images into the node body" with internal `assetIds[] + cursor + selectionMode`. Add `Text Iterator` (`strings[] + cursor + selectionMode`). Library multi-image drag drops onto the iterator surface instead of spawning N separate `File` nodes. Workflow-store `v7 → v8` migration converts existing `Image Iterator` instances by collecting wired upstream asset ids into the new internal array. Body grows the `<x/N>` counter + selection-mode dropdown.
- **Slice 5.6 — `Array`, `List`, `Number` nodes** — three pure / reactive nodes. `Array` splits a string by a delimiter. `List` selects one item from a list (single or array input) with optional cursor input handle for remote control. `Number` emits a number with the same `fixed | increment | decrement | random | range` mode vocabulary as iterators (used as a remote cursor). No engine changes.
- **Slice 5.7 — Run-here + per-node history** — adds the "▶" button next to `⋯` on every executable node header; engine grows `runWorkflow({ endAtNodeId? })` (~30 LOC: BFS reverse from `endAtNodeId`, topo sort over the subset). Adds `record.history?: HistoryEntry[]` (capped, per-kind override). The `<x/N>` counter in node bodies starts reading from history on past-run-aware nodes.
- **Slice 5.8 — SQLite via Drizzle** — finally cashes in the Repository abstraction (ADR-0005) so the workflow + execution stores survive page reloads.
- **Slice 5.4-deferred — queue panel grouping** — the "expand/collapse a pipeline that ran together" UX from the user's brief. Probably picks up alongside Slice 5.7 (history) since both are display-of-past-runs work.
