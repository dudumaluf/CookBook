...
- **Slice 5.6f (next, before Slice 5.7) — library polish** _(planned)_. Right-click context menu on cards (group / ungroup / detach / **duplicate group** (replaces the Detach button removed in Slice 5.6.1) / add to canvas / train Soul ID). Multi-delete on selected library cards via Backspace. Double-click rename on `image` and `soul-id` cards (same pattern as group cards in 5.6b). User flagged these mid-5.6c; addressed in their own sub-slice for clean reviewability.
- **Slice 5.7 — `Array`, `List`, `Number` nodes** — three pure / reactive nodes. `Array` splits a string by a delimiter. `List` selects one item from a list (single or array input) with optional cursor input handle for remote control. `Number` emits a number with the same `fixed | increment | decrement | random | range` mode vocabulary as iterators (used as a remote cursor). No engine changes.
- **Slice 5.8 — Run-here + per-node history** — adds the "▶" button next to `⋯` on every executable node header; engine grows `runWorkflow({ endAtNodeId? })` (~30 LOC: BFS reverse from `endAtNodeId`, topo sort over the subset). Adds `record.history?: HistoryEntry[]` (capped, per-kind override). The `<x/N>` counter in node bodies starts reading from history on past-run-aware nodes.
- **Slice 5.9 — SQLite via Drizzle** — finally cashes in the Repository abstraction (ADR-0005) so the workflow + execution stores survive page reloads.
- **Slice 5.4-deferred — queue panel grouping** — the "expand/collapse a pipeline that ran together" UX from the user's brief. Probably picks up alongside Slice 5.8 (history) since both are display-of-past-runs work.
