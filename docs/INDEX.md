...
1e. **[RESOLUTION-INSIGHTS.md](./RESOLUTION-INSIGHTS.md)** — cross-app idea bank: structured analysis of the Resolution storytelling platform (CONCEPT + platform-vision + LLM_RECIPES). Four directions parked for after Cookbook Library Phase B+. Each section stands alone, copy-paste-ready into another LLM. Sibling of `CATALOGO_E_IDEIAS_DE_NODES.md` (different source app, same role).
...
- **[STATE-AFTER-M1-media-arc.md](./STATE-AFTER-M1-media-arc.md)** — Multimodal media arc (Slices A-F): Seedance video + Continuity Builder (sequential iterator) + Video Concat + Fal image nodes + Video/Audio input + mediabunny WebCodecs ops + Performance Video recipe (ADR-0046 → ADR-0048). Includes the **test plan** (T1-T5). Soul ID training deferred.
- **[STATE-AFTER-M1-projects-arc.md](./STATE-AFTER-M1-projects-arc.md)** — Projects arc (Phases 1-4): surgical "run only this node" + project-as-document (per-node results/history persist + rehydrate on reload) + multi-project with per-project URLs (`/projetos/[id]`) + race-guarded ProjectSession + file portability (`.cookbook` JSON + self-contained `.zip`) (ADR-0049, ADR-0050).
- **[STATE-AFTER-cookbook-library.md](./STATE-AFTER-cookbook-library.md)** — Cookbook Library Phases A → E (recipe library + forks + version history + diff + prompt overrides + role overlays + role hand-off + recipe orchestration) **AND** Mega-capable assistant arc Tiers 0-3 (51 tools across 11 categories: chat memory, library mutations, recipe lifecycle, execution hygiene, graph chrome + repair; plus verifiable-precision pass with new `reactive-runner` / `assistant-store` tests + `propose_refactor → apply_pending_refactor` integration test). **Current.**
